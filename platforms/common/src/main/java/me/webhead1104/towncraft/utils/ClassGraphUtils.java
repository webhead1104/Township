        ...
        try (ScanResult result = graph.scan()) {
            result.getClassesImplementing(clazz).loadClasses().forEach(foundClass -> {
                try {
                    ...
                    Towncraft.getLogger().error("An error occurred whilst getting implemented classes for class '{}'!", clazz.getName(), e);
                }
            });
        } catch (Exception e) {
            Towncraft.getLogger().error("An error occurred whilst getting implemented classes for class '{}'!", clazz.getName(), e);
        ...
        try (ScanResult result = graph.scan()) {
            result.getSubclasses(clazz).loadClasses().forEach(foundClass -> {
                try {
                    ...
                    Towncraft.getLogger().error("An error occurred whilst getting extended classes for class '{}'!", clazz.getName(), e);
                }
            });
        } catch (Exception e) {
            Towncraft.getLogger().error("An error occurred whilst getting extended classes for class '{}'!", clazz.getName(), e);
