...
public enum BuildingType {
    ...
    TOWN_HALL(List.of(new Building(6, 0, 0, 0, new CoinPrice(0), null, null, StaticWorldTile.Type.WATER.getTile(), 0, TileSize.SIZE_2X2, "TOWN_HALL")));
    private final Map<Integer, Building> buildings = new HashMap<>();

    BuildingType(List<Building> buildings) {
        int i = 0;
    ...
    @Nullable
    public Building getNextPurchableBuilding(Player player) {
        User user = Township.getUserManager().getUser(player.getUniqueId());
        int amountPurchased = user.getPurchasedBuildings().amountPurchased(this);
        if (amountPurchased == -1) return null;
        if (amountPurchased == 0) {
            return buildings.get(amountPurchased);
        }
        return buildings.get(amountPurchased + 1);
    }
}
