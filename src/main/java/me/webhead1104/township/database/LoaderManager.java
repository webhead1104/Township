...
import me.webhead1104.township.Config;
...
public class LoaderManager {
    ...
    public LoaderManager() {
        Config config = Township.getTownshipConfig();
        Config.DatabaseConfig databaseConfig = config.getDatabase();

        userLoader = switch (databaseConfig.getDatabaseType()) {
            case FILE ->
                    new FileLoader(new File(Township.getInstance().getDataFolder(), databaseConfig.getFileConfig().getPath()));
            case MYSQL -> {
        ...
        if (userLoader instanceof UpdatableUserLoader updatableLoader) {
            try {
                ...
                Township.logger.error("Data source {} version is {}, while this loader version only supports up to version {}.",
                        databaseConfig.getDatabaseType().name(), e.getStorageVersion(), e.getImplementationVersion(), e);
            } catch (IOException e) {
                Township.logger.error("Failed to update data source {}", databaseConfig.getDatabaseType().name(), e);
            }
        }
    }
}
