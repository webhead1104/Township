...
public class MysqlLoader extends UpdatableUserLoader {
    ...
    public MysqlLoader(String sqlURL, String host, int port, String database, boolean useSSL, String username, String password) {
        try {
            ...
            Township.logger.error("Failed to establish connection to the MySQL server:", e);
        }
    }
    ...
    public void update() throws IOException, NewerStorageException {
        try (Connection con = source.getConnection()) {
            ...
            if (version < CURRENT_DB_VERSION) {
                ...
                LOGGER.warn("Note that this update might make your database incompatible with older Township versions.");
                LOGGER.warn("Make sure no other servers with older Township versions are using this database.");
