...
public class Factories {
    ...
    public static class Factory {
        ...
        public Factory() {
            for (int i = 0; i < 3; i++) {
                this.waiting.put(i, Towncraft.noneKey);
                this.completed.put(i, Towncraft.noneKey);
            }
            this.workingOn = Towncraft.noneKey;
        ...
        public RecipeType.Recipe removeFirstWaiting() {
            ...
            waiting.put(2, Towncraft.noneKey);
        ...
        public void addWaiting(Key key) {
            for (int i = 0; i < 3; i++) {
                if (!waiting.get(i).equals(Towncraft.noneKey)) continue;
                waiting.put(i, key);
                return;
            }
        }
        ...
        public boolean hasWaiting() {
            for (Key value : waiting.values()) {
                if (!value.equals(Towncraft.noneKey)) return true;
            }
            return false;
        }
        ...
        public void addCompleted(Key key) {
            ...
            for (Key value : completed.values()) {
                if (value.equals(Towncraft.noneKey)) {
                    completed.put(i, key);
                    return;
                }
                i++;
        ...
        public boolean canAddCompleted() {
            return completed.containsValue(Towncraft.noneKey);
        }
        ...
        public boolean canSetWorkingOn() {
            return workingOn.equals(Towncraft.noneKey);
        }
    }
}
