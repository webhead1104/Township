...
public class TowncraftView extends View {
    ...
    protected final State<User> userState = computedState(context -> Towncraft.getUserManager().getUser(context.getPlayer().getUniqueId()));
    protected MutableState<Class<? extends View>> closeClass = mutableState(WorldMenu.class);
    protected State<Object> initialData = computedState(context -> Towncraft.getUserManager().getUser(context.getPlayer().getUniqueId()).getSection());
