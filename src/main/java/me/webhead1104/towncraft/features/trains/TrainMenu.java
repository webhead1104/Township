        ...
        for (int i = 1; i < 4; i++) {
            ...
            for (int j = 1; j < 6; j++) {
                ...
                context.slot(carSlot++).updateOnClick().onRender(slotRenderContext -> {
                    ...
                    if (train.isClaimItems()) {
                        if (car.getClaimItemType().equals(Towncraft.noneKey)) {
                    ...
                    if (car.getGiveItemType().equals(Towncraft.noneKey)) {
                        ItemStack itemStack = ItemStack.of(Material.IRON_INGOT);
                    ...
                    if (train.isClaimItems()) {
                        ...
                        trainCar.setClaimItemType(Towncraft.noneKey);
                        trainCar.setClaimItemAmount(0);
                        AtomicBoolean good = new AtomicBoolean(true);
                        train.getTrainCars().forEach((key, value) -> {
                            if (!value.getClaimItemType().equals(Towncraft.noneKey) && value.getClaimItemAmount() != 0)
                                good.set(false);
                        });
                        if (good.get()) {
                        ...
                        if (trainCar.getGiveItemType().equals(Towncraft.noneKey)) return;
                        ...
                        if (user.getBarn().getItem(itemType) >= amount) {
                            ...
                            trainCar.setGiveItemType(Towncraft.noneKey);
                            AtomicBoolean good = new AtomicBoolean(true);
                            train.getTrainCars().forEach((key, value) -> {
                                if (!value.getGiveItemType().equals(Towncraft.noneKey) && value.getGiveItemAmount() != 0)
                                    good.set(false);
                            });
                            if (good.get()) {
                                Trains.Train newTrain = new Trains.Train(finalI);
