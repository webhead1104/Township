...
public class BarnMenu extends TowncraftView {
    ...
    private final MutableState<Key> sellItem = mutableState(Towncraft.noneKey);
    ...
    public void onFirstRender(@NotNull RenderContext context) {
        ...
        for (Key item : barnState.get(context).getItemMap().keySet()) {
            if (item.equals(Towncraft.noneKey)) continue;
            context.availableSlot().updateOnStateChange(sellAmount).hideIf(hideContext -> barnState.get(hideContext).getItem(item) <= 0)
                    .onRender(slotRenderContext -> {
                        ItemStack stack = Towncraft.getDataLoader(ItemType.class).get(item).getItemStack();
                        stack.setData(DataComponentTypes.ITEM_NAME, Msg.format("%s: <yellow>%d", Towncraft.getDataLoader(ItemType.class).get(item).getName(), barnState.get(slotRenderContext).getItem(item)));
        ...
        context.slot(48).hideIf(hideContext -> sellItem.get(hideContext).equals(Towncraft.noneKey) || sellAmount.get(hideContext) == 1)
        ...
        context.slot(49).hideIf(hideContext -> sellItem.get(hideContext).equals(Towncraft.noneKey))
                .updateOnStateChange(sellAmount).onRender(slotRenderContext -> {
                    ...
                    sellItem.set(Towncraft.noneKey, slotClickContext);
                    sellAmount.set(1, slotClickContext);
                    slotClickContext.openForPlayer(BarnMenu.class);
                });

        context.slot(50).hideIf(hideContext -> sellItem.get(hideContext).equals(Towncraft.noneKey) ||
    ...
    private void setInventoryItems(Context context) {
        ...
        barn.getItemMap().forEach((key, value) -> {
            if (value != 0) {
                storageLore.addLine(Msg.format("<white>%s: <yellow>%d", Towncraft.getDataLoader(ItemType.class).get(key).getName(), value));
            }
        });
